...
import 'package:e_commers/Models/Response/response_user_registration.dart';
...
class UserServices {
  Future<ResponseUserRegistration> addNewUser(
      String username, String email, String password) async {
    final bodyMessage =
        jsonEncode({'name': username, 'email': email, 'password': password});
    final resp = await http.post(Uri.parse('${URLS.baseUrl}user/register'),
        headers: {'Content-Type': 'application/json'}, body: bodyMessage);

    return ResponseUserRegistration.fromJson(
        jsonDecode(resp.body), resp.statusCode);
  }
