  ...
  Widget build(BuildContext context){
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Custom_Text(text: 'Purchased', color: Colors.black87, fontWeight: FontWeight.w500, fontSize: 20 ),
        centerTitle: true,
        elevation: 0,
        leading: IconButton(
          splashRadius: 20,
      ...
      itemBuilder: (_, i) 
        => InkWell(
          ...
          child: Container(
            ...
            children: [
              Custom_Text(text: ordersBuy[i].receipt, fontSize: 21, color: ColorsCustom.kprimaryColor, fontWeight: FontWeight.w500),
              const SizedBox(height: 10.0),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Custom_Text(text: 'Date ', fontSize: 18, color: Colors.grey ),
                  Custom_Text(text: timeago.format(ordersBuy[i].createdAt, locale: 'es'), fontSize: 18 ),
                ],
              ),
              const SizedBox(height: 10.0),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Custom_Text(text: 'Amount ', fontSize: 18, color: Colors.grey ),
                  Custom_Text(text: '\$ ${ordersBuy[i].amount}', fontSize: 20, fontWeight: FontWeight.w500 ),
                ],
              ),
            ],
          ),
