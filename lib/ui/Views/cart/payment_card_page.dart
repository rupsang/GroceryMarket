...
class PaymentCardPage extends StatelessWidget {
  ...
  Widget build(BuildContext context){
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: const Custom_Text(text: 'Payment', color: Colors.black, fontSize: 21, fontWeight: FontWeight.bold),
        elevation: 0,
        centerTitle: true,
        leading: IconButton(
          splashRadius: 20,
        ...
        actions: [
          TextButton(
            onPressed: (){},
            child: Custom_Text(text: 'Add Card', color: ColorsCustom.kprimaryColor, fontSize: 17),
          )
        ],
      ),
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: state.creditCardFrave == null 
                        ? Colors.black
                        : state.creditCardFrave!.cvv == card.cvv ? ColorsCustom.bgcolor : Colors.black
                    )
                  ),
                  child: Row(
                    ...
                    children: [
                      ...
                      Container(
                        child: Custom_Text(text: '**** **** **** ${card.cardNumberHidden}')
                      ),
                      Container(
                        child: state.creditCardFrave == null 
                        ? Icon(Icons.radio_button_off_rounded, size: 31)
                        : state.creditCardFrave!.cvv == card.cvv 
                          ? Icon(Icons.radio_button_checked_rounded, size: 31, color: ColorsCustom.kprimaryColor,)
                          : Icon(Icons.radio_button_off_rounded, size: 31)
                      )
                    ],
                  ),
